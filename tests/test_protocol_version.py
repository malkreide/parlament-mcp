...
def test_die_geloggte_revision_ist_die_des_sdk_und_kein_literal() -> None:
    """`server_start` loggt `PROTOCOL_VERSION`. Der Wert stand drei Revisionen
    lang auf `2025-06-18`, waehrend der Server `2025-11-25` aushandelte — ein
    Log, das etwas anderes sagt als die Leitung, ist beim Debuggen schlimmer
    als gar keines.

    Die Konstante wird jetzt abgeleitet. Dieser Test faengt den Tag ab, an dem
    jemand wieder ein Literal hinschreibt: er vergleicht mit dem SDK, nicht mit
    sich selbst.
    """
